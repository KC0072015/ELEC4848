...
def setup_retriever():
    """Build the same MMR retriever used by api.py."""
...
def score_refusal(response: str, should_refuse: bool) -> dict:
    ...
    """
    rl = response.lower()
    is_refusal = any(p in rl for p in REFUSAL_PHRASES)
    if should_refuse:
        return {
...
def run_evaluation(
    ...
    for session in dataset["sessions"]:
        ...
        for td in session["turns"]:
            ...
            response, elapsed = collect_response(query, retriever, history, verbose)
